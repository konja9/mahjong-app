...
/** 図柄は萬子の 1〜9（index 0〜8）。奇数図柄は確変 */
export const isKakuhenSymbol = (sym: number): boolean => sym % 2 === 0;
export const PREMIUM_SYMBOL = 6; // 7萬
...
export class Machine {
  ...
  rush = false;
  stLeft = 0;
  data: MachineData = { spins: 0, hits: 0, rushChain: 0, maxChain: 0, rushEntries: 0 };
  ...
  spin(): SpinResult | null {
    ...
    if (hold.hit) {
      const pool = [0, 1, 2, 3, 4, 5, 6, 7, 8].filter((s) => isKakuhenSymbol(s) === hold.kakuhen);
      const s = pool[Math.floor(this.rng() * pool.length)];
      symbols = [s, s, s];
    } else if (reach) {
      const s = Math.floor(this.rng() * 9);
      symbols = [s, (s + (this.rng() < 0.5 ? 1 : 8)) % 9, s];
    } else {
      const l = Math.floor(this.rng() * 9);
      let r = Math.floor(this.rng() * 9);
      if (r === l) r = (r + 1 + Math.floor(this.rng() * 7)) % 9;
      symbols = [l, Math.floor(this.rng() * 9), r];
    }
  ...
  reset(): void {
    ...
    this.rush = false;
    this.stLeft = 0;
    this.data = { spins: 0, hits: 0, rushChain: 0, maxChain: 0, rushEntries: 0 };
  }
}
