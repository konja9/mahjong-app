/**
 * 交換所：台の解放・景品（見た目だけ）・ミッション。
 * 保存は tensu.shop.v1。破産しても消えない（購入は恒久的な yan の使い道）
 */
...
export type ShopTab = 'machine' | 'items' | 'missions';

const yen = (n: number) => `${n.toLocaleString()} yan`;

/** 交換所ダイアログの中身 */
export function shopHtml(s: ShopState, tab: ShopTab, balance: number, canSwitch: boolean): string {
  const tabs = ([['machine', '台'], ['items', '景品'], ['missions', 'ミッション']] as [ShopTab, string][])
    .map(([t, l]) => `<button class="cfg${t === tab ? ' on' : ''}" data-shop-tab="${t}">${l}</button>`)
    .join('');
  let body = '';
  if (tab === 'machine') {
    body = MACHINE_IDS.map((id) => {
      const sp = SPECS[id];
    ...
    }).join('') + (canSwitch ? '' : '<p class="help-note">BONUS 中と台が回っている間は、台を切り替えられません。</p>') +
      '<p class="help-note">上の台ほど大当りは重いが、BONUS が長く賞金が大きい。振れ幅が大きく、実力のある人ほど大きく稼げます。</p>';
  } else if (tab === 'items') {
    const group = (kind: ItemKind, title: string) =>
      ...
      ITEMS.filter((i) => i.kind === kind)
        ...
        .join('');
    body = group('back', '牌の背') + group('skin', '液晶のスキン') + group('title', '称号');
  } else {
  ...
  return `<div class="settings help shop">
    <div class="set-head"><span>交換所</span><span class="shop-wallet">所持 <b>${balance.toLocaleString()}</b> yan</span><button class="icon-btn" data-shop-close aria-label="閉じる">×</button></div>
    <div class="cfg-group help-tabs" role="tablist">${tabs}</div>
